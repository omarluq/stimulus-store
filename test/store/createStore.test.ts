...
describe('createStore', () => {
  it('should throw an error if no initial value is provided', () => {
    expect(() => createStore({ name: 'testStore', type: Number } as any)).toThrow(
  ...
  it('should throw an error if an invalid type is provided', () => {
    expect(() => createStore({ name: 'testStore', type: Set, initialValue: 0 } as any)).toThrow('Invalid type: Set')
  })
  ...
  it('should create a store with a string type', () => {
    const store = createStore({ name: 'testStore', initialValue: 'test', type: String })
    expect(typeof store.get()).toBe('string')
  })

  it('should create a store with a boolean type', () => {
    const store = createStore({ name: 'testStore', initialValue: true, type: Boolean })
    expect(typeof store.get()).toBe('boolean')
  })

  it('should create a store with an array type', () => {
    const store = createStore({ name: 'testStore', initialValue: [1, 2, 3], type: Array })
    expect(Array.isArray(store.get())).toBe(true)
  })

  it('should create a store with an object type', () => {
    const store = createStore({ name: 'testStore', initialValue: { key: 'value' }, type: Object })
